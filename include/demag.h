...
namespace tomocam::opt {
    ...
    std::array<Array<T>, 3> demag(const std::array<Array<T>, 3> &m) {
        ...
        // For R2C FFT, only compute positive frequencies in z
        std::vector<T> kz(dims.n3 / 2 + 1);
        for (size_t i = 0; i < dims.n3 / 2 + 1; ++i) {
            kz[i] = static_cast<T>(i) / static_cast<T>(dims.n3);
        }
        ...
        for (size_t i = 0; i < dims.n1; ++i) {
            for (size_t j = 0; j < dims.n2; ++j) {
                // Compute base flat index for this (i,j) slice, then increment for k
                size_t base_idx = (i * dims.n2 + j) * nk;
                
                for (size_t k = 0; k < nk; ++k) {
                    size_t flat_idx = base_idx + k;
                    ...
                    // Pre-compute reciprocal to replace 3 divisions with 1 division + 3 multiplications
                    ...
                    std::complex<T> k_dot_m_hat = kx[i] * m0 + ky[j] * m1 + kz[k] * m2;
        ...
        for (size_t comp = 0; comp < 3; ++comp) {
            demag_hat[comp][{0, 0, 0}] = 0.0;
        }

        std::array<Array<T>, 3> H;
        dims_t real_dims = {dims.n1, dims.n2, dims.n3};
        for (size_t comp = 0; comp < 3; ++comp) {
            H[comp] = fft::fft3_c2r(demag_hat[comp], real_dims);
        }
