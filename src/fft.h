...
namespace tomocam {
    namespace fft {
        ...
        template <typename T> Array<std::complex<T>> fft1(const Array<std::complex<T>> &input) {
            ...
            if (is_double) {
                ...
                fftw_plan plan =
                    fftw_plan_many_dft(rank, n, batches, idata, inembed, istride, idist, odata,
                ...
                fftwf_plan plan =
                    fftwf_plan_many_dft(rank, n, batches, idata, inembed, istride, idist, odata,
                                        onembed, ostride, odist, FFTW_FORWARD, FFTW_ESTIMATE);
                fftwf_execute(plan);
        ...
        template <typename T> Array<std::complex<T>> ifft1(const Array<std::complex<T>> &arr) {
        ...
        template <typename T> Array<std::complex<T>> fft2(const Array<std::complex<T>> &input) {
        ...
        template <typename T> Array<std::complex<T>> ifft2(const Array<std::complex<T>> &input) {
            // double or float
            bool is_double = std::is_same_v<T, double>;
        ...
        template <typename T> Array<std::complex<T>> fft3(const Array<std::complex<T>> &input) {
            ...
            if (is_double) {
                ...
                fftw_complex *odata = (fftw_complex *)output.begin();
                fftw_plan plan =
                    fftw_plan_dft_3d(dims.x(), dims.y(), dims.z(), idata, odata, -1, FFTW_ESTIMATE);
                if (!plan) {
                    throw std::bad_alloc();
                }
                fftw_execute(plan);
                ...
                fftwf_complex *idata = (fftwf_complex *)input.begin();
                fftwf_complex *odata = (fftwf_complex *)output.begin();
                fftwf_plan plan = fftwf_plan_dft_3d(dims.x(), dims.y(), dims.z(), idata, odata, -1,
                                                    FFTW_ESTIMATE);
                if (!plan) {
                    throw std::bad_alloc();
                }
        ...
        template <typename T> Array<std::complex<T>> ifft3(const Array<std::complex<T>> &input) {
            ...
            if (is_double) {
                ...
                fftwf_complex *odata = (fftwf_complex *)output.begin();
                fftwf_plan plan =
                    fftwf_plan_dft_3d(dims.x(), dims.y(), dims.z(), idata, odata, 1, FFTW_ESTIMATE);
                if (!plan) {
                    throw std::bad_alloc();
                }
                fftwf_execute(plan);
